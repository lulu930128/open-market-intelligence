...
SCHEDULER_WATCHLIST_RADAR_USE_INTRADAY=false
...
Watchlist Radar 自動快照是本機 SQLite job，預設在台北時間 15:45 跑，先評估近期已存的舊快照，再保存當日 `action` mode 快照。`SCHEDULER_WATCHLIST_RADAR_GROUP_IDS` 留空時只掃描 active root watchlist group，並用 `SCHEDULER_WATCHLIST_RADAR_INCLUDE_CHILDREN=true` 納入子群；若要固定只跑畫面上的群組，可填逗號分隔 group id。`SCHEDULER_WATCHLIST_RADAR_REQUIRE_DAILY_RELEASE=true` 會在台股日線 release window 前跳過，避免每天自動保存 stale snapshot。
