...
def _kr_close_value(row: KRDailyPrice | None) -> float | None:
    if row is None:
        return None
...
def _chart_row(row: KRDailyPrice, time_value: date | None = None) -> dict:
    return {
...
def _aggregate_kr_daily_rows(rows: list[KRDailyPrice], timeframe: str) -> list[dict]:
    if timeframe == "daily":
    ...
    for bucket_key, bucket_rows in buckets.items():
        ...
        closes = [_kr_close_value(row) for row in sorted_rows if _kr_close_value(row) is not None]
...
def list_kr_ohlc_chart_data(
    ...
    *,
    ...
    backfill_result = None
    if ensure_history:
        backfill_result = refresh_kr_daily_prices(
            db=db,
            symbol=normalized_symbol,
            outputsize=outputsize,
            provider=provider,
    ...
    rows = (
        ...
        .order_by(KRDailyPrice.trade_date.asc(), KRDailyPrice.provider.asc(), KRDailyPrice.id.asc())
        .all()
    )
    ...
    for row in rows:
        ...
        if current is None or provider_priority.get(row.provider, 99) < provider_priority.get(current.provider, 99):
            latest_by_date[row.trade_date] = row
    points = _aggregate_kr_daily_rows(list(latest_by_date.values()), timeframe)[-bars:]
