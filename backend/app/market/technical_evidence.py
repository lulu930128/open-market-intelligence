...
def _wilder_rsi(
    ...
    for index in range(1, len(closes)):
        ...
        if average_loss == 0:
            ...
            results[index] = _round(100 - 100 / (1 + rs))
    return results
...
def _macd_sma_seed(
    ...
    *,
    ...
    fast = _ema_sma_seed(closes, fast_period)
    slow = _ema_sma_seed(closes, slow_period)
    macd = [
        _round(fast_value - slow_value)
        if fast_value is not None and slow_value is not None
        else None
        for fast_value, slow_value in zip(fast, slow)
    ]
    signal = _ema_sma_seed(macd, signal_period)
    histogram = [
...
def _recursive_kd(
    ...
    *,
    ...
    for index, close in enumerate(closes):
        ...
        current_rsv = 50.0 if highest == lowest else (close - lowest) / (highest - lowest) * 100
        ...
        rsv[index] = _round(current_rsv)
        k_values[index] = _round(current_k)
        d_values[index] = _round(current_d)
        j_values[index] = _round(current_j)
...
def _pvo(
    ...
    *,
    ...
    fast = _ema_sma_seed(normalized, fast_period)
    slow = _ema_sma_seed(normalized, slow_period)
    values = [
        _round((fast_value - slow_value) / slow_value * 100)
        if fast_value is not None and slow_value not in {None, 0}
        else None
        for fast_value, slow_value in zip(fast, slow)
    ]
    signal = _ema_sma_seed(values, signal_period)
    histogram = [
        _round(value - signal_value)
        if value is not None and signal_value is not None
        else None
