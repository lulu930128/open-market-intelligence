...
TW_DATASET_OPERATIONS = (
    ...
    TaiwanDatasetOperationSpec(
        operation_id="tw.refresh_index_intraday_bars",
        callable_path="app.market.tw_current_market_operations.refresh_taiwan_index_intraday_bars_operation",
        external_io=True,
        writes_storage=True,
        bounds=_bounds(2, 40, 1, 1),
        limitations=("CURRENT_SESSION_ONLY", "PROVISIONAL_EVIDENCE"),
    ...
    TaiwanDatasetOperationSpec(
        operation_id="tw.refresh_market_index_directory",
        callable_path="app.market.indices.refresh_market_index_list",
        external_io=True,
        writes_storage=True,
        bounds=_bounds(4, 40, 1, 1),
        limitations=("OFFICIAL_ENDPOINTS_BEST_EFFORT", "VALUES_MAY_BE_PARTIAL"),
    ),
    TaiwanDatasetOperationSpec(
...
TW_DATASET_CONTRACTS = (
    ...
    _dataset(
        ...
        frequency=TaiwanDatasetFrequency.EVENT,
        expected=TaiwanExpectedStatePolicy.CURRENT_SESSION,
        eligibility="listed_instrument_active_or_close_resolution_session",
        refresh_operation="tw.refresh_realtime_snapshot",
        refresh_bounds=_bounds(3, 40, 1, 1),
        repairable=False,
        postcondition="Repository reread resolves a bounded canonical live order book or a same-session closing snapshot with provider, source, event time, raw receipt, quality, and explicit non-tradable semantics.",
        lineage=TaiwanDatasetLineageStatus.CANONICAL_RAW_RECEIPT,
        lineage_fields=_CANONICAL_LINEAGE,
        convergence=TaiwanDatasetConvergenceStatus.PLATFORM_OWNED,
        limitations=(
            "ACTIVE_OR_CLOSE_RESOLUTION_CAPTURE_ONLY",
            "POST_CLOSE_CACHE_ONLY",
            "LEVEL_5_MAXIMUM",
        ),
    ),
    _dataset(
    ...
    _dataset(
        dataset_id="tw.market_index.intraday",
        family="market_state",
        payload="tw.bar.series_read.v1",
        scope="TAIEX_or_TPEX",
        capabilities=("market.index.intraday",),
        tables=(
            "source_registry",
            "raw_fetch_result",
            "market_intraday_bar",
        ...
        read="app.market.tw_bar_service.read_taiwan_index_intraday_bars",
        projection="app.market.tw_bar_service.project_taiwan_index_intraday_bars",
        frequency=TaiwanDatasetFrequency.INTRADAY,
        expected=TaiwanExpectedStatePolicy.CURRENT_SESSION,
        eligibility="current_taiwan_session_and_supported_index",
        refresh_operation="tw.refresh_index_intraday_bars",
        refresh_bounds=_bounds(2, 40, 1, 1),
        repairable=False,
        postcondition="Reread canonical index Bars and resolve one deterministic minute series through the unified Bar Gateway.",
        lineage=TaiwanDatasetLineageStatus.DERIVED_COMPONENT_LINEAGE,
        lineage_fields=(
            *_CANONICAL_LINEAGE,
            "source_interval",
    ...
    _dataset(
        dataset_id="tw.market_index.directory",
        family="market_state",
        payload="tw.market_index.directory.v1",
        scope="TWSE_or_TPEX_index_directory",
        capabilities=("market.index.directory",),
        tables=(
            "source_registry",
            "raw_fetch_result",
            "taiwan_market_index_directory_snapshot",
            "taiwan_market_index_directory_item",
        ),
        read="app.market.tw_index_directory.read_taiwan_index_directory",
        projection="app.routers.tw_market_indices.get_indices_list",
        frequency=TaiwanDatasetFrequency.INTRADAY,
        expected=TaiwanExpectedStatePolicy.CURRENT_SESSION,
        eligibility="supported_market_and_latest_successful_directory_snapshot",
        refresh_operation="tw.refresh_market_index_directory",
        refresh_bounds=_bounds(4, 40, 1, 1),
        repairable=True,
        postcondition="Persist a complete versioned directory snapshot atomically, retain the previous latest-successful snapshot on refresh failure, and reread transport plus expected-observation-date freshness independently.",
        lineage=TaiwanDatasetLineageStatus.CANONICAL_RAW_RECEIPT,
        lineage_fields=_CANONICAL_LINEAGE,
        convergence=TaiwanDatasetConvergenceStatus.PLATFORM_OWNED,
        limitations=(
            "DIRECTORY_VALUES_MAY_BE_PARTIAL",
            "TRANSPORT_FRESHNESS_DOES_NOT_IMPLY_OBSERVATION_CURRENT",
        ),
    ),
    _dataset(
