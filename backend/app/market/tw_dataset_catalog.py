...
TW_DATASET_CONTRACTS = (
    ...
    _dataset(
        ...
        family="market_state",
        payload="tw.market_index.intraday.v1",
        scope="TAIEX_or_TPEX",
        ...
        read="app.market.tw_current_market_platform.read_taiwan_index_intraday_series",
        projection="app.market.tw_current_market_platform.project_taiwan_index_intraday_series",
        frequency=TaiwanDatasetFrequency.INTRADAY,
        expected=TaiwanExpectedStatePolicy.CURRENT_SESSION,
        eligibility="current_taiwan_session_and_supported_index",
        refresh_operation="tw.refresh_current_index",
        refresh_bounds=_bounds(2, 40, 1, 1),
        repairable=False,
        postcondition="Resolve lineage-compatible canonical current-index events by minute without reading or promoting the legacy minute snapshot table.",
        lineage=TaiwanDatasetLineageStatus.DERIVED_COMPONENT_LINEAGE,
        lineage_fields=(*_CANONICAL_LINEAGE, "minute_at", "component_raw_result_ids"),
        convergence=TaiwanDatasetConvergenceStatus.PLATFORM_OWNED,
        limitations=("CURRENT_SESSION_PROVISIONAL", "HISTORICAL_GAPS_NOT_REPAIRABLE"),
    ),
    _dataset(
        ...
        repairable=True,
        postcondition="Persist a complete versioned directory snapshot atomically and retain the previous latest-successful snapshot on refresh failure.",
        lineage=TaiwanDatasetLineageStatus.CANONICAL_RAW_RECEIPT,
        lineage_fields=_CANONICAL_LINEAGE,
        convergence=TaiwanDatasetConvergenceStatus.PLATFORM_OWNED,
        limitations=("DIRECTORY_VALUES_MAY_BE_PARTIAL",),
    ),
    _dataset(
