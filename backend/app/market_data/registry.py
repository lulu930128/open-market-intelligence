...
INTERNAL_DATASET_REFRESH_OPERATIONS = frozenset(
    {
        ...
        "tw.refresh_index_intraday_bars",
        "tw.refresh_market_index_directory",
...
DATASET_REGISTRY = DatasetRegistry(
    (
        ...
        DatasetSpec(
            ...
            postcondition="Persist a canonical bounded order book and raw receipt, reread, and return live depth or an explicitly non-tradable same-session closing snapshot; otherwise return truthful missing, stale, partial, or policy-unsatisfied evidence.",
        ),
        DatasetSpec(
        ...
        DatasetSpec(
            ...
            schema_version="tw.bar.series_read.v1",
            market=Market.TW,
            scope_kind="market_index",
            owner="app.market.tw_bar_service",
            read_operation="read_taiwan_index_intraday_bars",
            projection_id="market.index.intraday.index.TW",
            capability_ids=("market.index.intraday",),
            frequency=DatasetFrequency.INTRADAY,
            expected_state_policy=ExpectedStatePolicy.CURRENT_SESSION,
            eligibility_policy=EligibilityPolicy.MARKET_TRADING_DAY,
            storage_reference="source_registry+raw_fetch_result+market_intraday_bar+market_intraday_bar_lineage",
        ...
        DatasetSpec(
            ...
            schema_version="tw.market_index.directory.v1",
            market=Market.TW,
            scope_kind="market_index_directory",
            owner="app.market.tw_index_directory",
            read_operation="read_taiwan_index_directory",
            projection_id="market.index.directory.market.TW",
            capability_ids=("market.index.directory",),
            frequency=DatasetFrequency.INTRADAY,
            expected_state_policy=ExpectedStatePolicy.CURRENT_SESSION,
            eligibility_policy=EligibilityPolicy.MARKET_TRADING_DAY,
            storage_reference="source_registry+raw_fetch_result+taiwan_market_index_directory_snapshot+taiwan_market_index_directory_item",
            refreshable=True,
            ...
            postcondition="Persist one complete versioned directory snapshot atomically, preserve the previous latest-successful snapshot on failure, and reread transport plus expected-observation-date freshness independently.",
            repairable=True,
        ),
        DatasetSpec(
        ...
        DatasetSpec(
            ...
            market=Market.US,
            scope_kind="us_stock",
            owner="app.us_market.intraday_platform.USIntradayMarketPlatform",
            read_operation="read_quote",
            projection_id="quote.snapshot.us_stock.US",
            capability_ids=("quote.snapshot",),
            frequency=DatasetFrequency.EVENT,
            expected_state_policy=ExpectedStatePolicy.CURRENT_SESSION,
            eligibility_policy=EligibilityPolicy.LISTED_INSTRUMENT_AND_TRADING_DAY,
            storage_reference="source_registry+raw_fetch_result+us_quote_snapshot",
            refreshable=True,
            ...
            postcondition="Persist and reread one resolved canonical quote snapshot with explicit lineage and limitations.",
        ),
        DatasetSpec(
            ...
            scope_kind="us_stock",
            owner="app.us_market.intraday_platform.USIntradayMarketPlatform",
            read_operation="read_intraday_bars",
            ...
            postcondition="Persist and reread resolved canonical intraday bars with legacy lineage-free rows rejected.",
        ),
        DatasetSpec(
