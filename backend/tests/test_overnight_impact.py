...
class OvernightImpactTests(unittest.TestCase):
    ...
    def test_tw_stock_tool_session_refreshes_missing_overnight_factors(self) -> None:
        ...
        with patch(
            "app.ai.agentic_tools.us_market_service.refresh_us_daily_prices",
            return_value={
                "status": "success",
                "provider": "yahoo_chart",
                "symbol": "^SOX",
        ...
        self.assertTrue(
            all(run["tool"] == "us.refresh_daily_price" for run in session["tool_runs"])
        )
        self.assertEqual(
            {run["arguments"]["symbol"] for run in session["tool_runs"]},
            set(overnight_gaps["refresh_symbols"][:2]),
        )
