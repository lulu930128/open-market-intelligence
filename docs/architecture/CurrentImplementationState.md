...
| JP shared-market convergence | in_progress | daily_adopted | completed_daily_provider_verified | daily_mcp_verified | 2026-09-12 | `backend/tests/test_jp_canonical_daily.py`; `backend/tests/test_jp_session_reference.py`; `docs/exec-plans/active/jp-shared-market-convergence-20260912/Acceptance.md` | 正式 migration 至 0086，JP daily reader 已 on；Yahoo daily receipt／history 與 REST／proxy／獨立 HTTP MCP 已驗證，4384.T 保留 missing。盤中 bar-close reference／MCP selection／競價 gap 持續收束；J-Quants entitlement、canonical intraday、非價格 Research、完整 retirement 與 live session gates 未完成 |
| KR shared-core convergence | in_progress | partial | not_reverified | partial | 2026-09-12 | `backend/tests/test_kr_read_purity.py`; `backend/tests/test_kr_canonical_daily.py`; `docs/exec-plans/active/kr-shared-core-convergence-20260912/` | Daily consumer gated binding 與 offline AI／MCP lineage parity 通過；external MCP cache-only read 可回 truthful stale／missing。正式 schema 唯讀觀察為 0086、KR canonical 0 rows；rollout 未切換。Index／Intraday full canonical、非價格 dataset、venue coverage、KIS、正式 consumer adoption 與 legacy removal 未完成 |
...
- Canonical owner：`ResolvedTaiwanIndexTruth`升為`tw.index.resolution.v4`，只組合已解析的current／intraday／completed daily／official-close evidence role，不擁有provider priority、I/O或storage winner selection；selected value、previous close、change與change percentage保留同一semantic lane及reference lineage，且previous close以selected trade date的前一交易日驗證`current／stale／missing／unknown`。REST summary、Dashboard、AI `market.indices`、AI compact quote與index contract snapshot共用同一resolution identity；`current_data_core.index`只保留diagnostic／live snapshot與明示版本、limitation的短期compatibility fallback。
...
- Runtime projection（2026-09-02 checkpoint）：當時cache-only GET已回`tw.index.resolution.v3`；2026-09-04 source已升為v4並加入typed previous-close status，必須以本輪隔離runtime或正式launcher重新證明adoption。先前PID與identity證據不可沿用為v4 Runtime acceptance。
...
## 2026-09-03 Taiwan current-session consumer convergence Source checkpoint

- Session owner：`TaiwanBarService.read_current_session_bars()`以`taiwan_presentation_session()`決定單一presentation trade date及09:00起的bounded window；Chart bundle新增`session_scope=current_session`與`presentation_trade_date`。Frontend Today只送intent，不從latest cached bar推交易日，refresh也只在同一trade date合併；Backend identity缺失或混入其他日期時fail visible。一般professional／historical intraday仍可跨session。
- Consumer cutover：Current market summary實際production read path與AI market index intraday pack都改讀`tw.market_index.intraday`的Unified Bar owner；dead summary attachment seam已移除，production consumer negative guard禁止回接`read_taiwan_index_minute_series`。Index resolution沿用summary的previous-close reference計算同一semantic lane的change，不新增provider或storage owner。
- AI quality：Current-session series帶出`expected_trade_date`；generic Data Quality會掃描所有nested index series，任何日期不一致都投影`CURRENT_SESSION_SERIES_DATE_MISMATCH`、coverage partial、`decision_usable=false`及`intraday_research_usable=false`。未宣告current-session的歷史multi-session series不受此gate影響。
- Source evidence：affected Backend matrix `175 passed / 12 subtests`；safe quick的architecture checker、architecture pytest、Backend compileall、Frontend TypeScript與`git diff --check`通過，log為`.tmp/validation/20260903-100453/`；targeted Frontend ESLint與TPEX Today Playwright `1 passed`。Current-source／production-SQLite cache-only readback在2026-09-03盤中確認3711、TAIEX、TPEX各自只含09/03，summary選到`intraday_last_trade`，AI pack的scope／expected date為current-session／09/03且兩個index series均只含09/03。
- Acceptance boundary：本checkpoint是Source與current-source cache-only evidence；沒有provider I/O、DB mutation、runtime restart、commit或push。Running 8400的launcher設定為`backend_reload=False`，因此Backend runtime adoption、正式transport／MCP與使用者實盤Product acceptance仍pending。Frontend production build因3000 dev runtime正在使用同一`.next`目錄而未執行，已由TypeScript、targeted ESLint及重用3000的focused Playwright覆蓋本次互動風險。

## 2026-09-03 Taiwan stock intraday pipeline convergence Source checkpoint

- Current-session resolution：台股Stock／ETF的current-session Base-1m改為cache-only `COMPOSE_BY_TIMESTAMP`。Resolver依單一minute identity局部選winner，只接受同presentation trade date、FINAL／CORRECTED且已關閉的bucket；KGI live tail不再整段shadow NStock／Yahoo baseline，active minute仍不進final series。
- Provider temporal boundary：`kgi.superpy.minute_kbars.v2`把provider callback timestamp視為bucket end並轉為canonical start，同時保留raw provider time；2026-09-03 production SQLite的20組KGI v2／NStock同minute樣本有19組OHLCV完全一致、1組不一致，因此一分鐘校正維持明確limitation而不宣稱完全等價。KGI `total_amount`依provider文件標成累計成交金額，只留raw receipt，不投影為minute turnover。Launcher runtime另暴露Yahoo `:10`／`:11`秒級bucket與NStock整分鐘bucket重疊；`yahoo.chart.v8.intraday.v2`改由provider adapter對齊minute grid並保留raw time，舊Yahoo／KGI v1與任何非minute-grid row均由binding／repository fail closed，沒有刪除或改寫production DB。
- Coverage／repair／presentation：Bar response新增typed current-session coverage與missing ranges，repair operation引用executable catalog的`tw.refresh_intraday_bars`。Frontend不再從read lifecycle自動POST repair，也不把理論expected bucket count當provider route gate；bounded Tier-A repair／materialization維持Backend scheduler owner、最短60秒週期、coalesce與single-instance。Today presentation cache是最多10 symbols的LRU，必須符合Backend calendar snapshot的presentation trade date，命中後先畫cache再用cache-only GET revalidate。
- Daily／Resolver failure boundary：timestamp composition逐Bar做hard eligibility，candidate最新一根hard-invalid不會抹除先前合法prefix。Current provisional Daily component若仍重疊，只略過當日derived projection並回`TW_CURRENT_SESSION_DAILY_PROJECTION_INVALID`；completed Daily history與其他contract failure不被吞掉。Watchlist ranking schema接受datetime並投影ISO 8601，修正與Daily overlap無關的`latest-batch` response validation 500。
- Lease edge：KGI viewer lease release前會先best-effort materialize buffer中所有已關閉minute，再釋放provider handle；flush失敗會記warning且不阻斷release。GET/read path維持零provider I/O、零enqueue、零寫入。
- Source validation：corrective affected Backend matrix `176 passed`；Frontend TypeScript、targeted ESLint與focused source-contract Playwright `7 passed`。最終Safe quick的architecture checker、architecture pytest、Backend compileall、Frontend TypeScript及`git diff --check`全數通過，log為`.tmp/validation/20260903-125838/`。Current-source／production-SQLite cache-only probe於12:58讀到3711 current 237 bars、0 overlap、contributors為NStock＋KGI v2；Daily回260 bars並含09/03 provisional。Watchlist group 45的4筆ranking可由新版schema序列化，first time為timezone-aware ISO 8601。
- Acceptance boundary：本checkpoint代表Source與current-source production-shaped cache-only acceptance；未執行provider I/O、production DB mutation、runtime restart、running OpenAPI adoption、正式盤中多次換股／延遲量測、MCP Product parity、production build、commit或push。KGI公開文件未明示KBar datetime是bucket start或end，且樣本仍有1組OHLCV差異，因此limitations與parser version必須持續保留；正式Live／Product gate仍需在新runtime逐bucket交叉驗證。

### Today snapshot Ready Gate follow-up

- Backend owner：`tw.bar.current_session_coverage.v2`由`TaiwanBarService`投影`warming | ready | degraded`、reason codes與完整snapshot revision／bar count／bounds；snapshot identity在response套用`limit`前建立，因此full與latest-8可判斷同一canonical snapshot。`trailing_window`不再等同可發布圖表；sparse只有在至少80% observed且missing不超過`max(3, expected/20向上取整)`時才degraded，過度稀疏仍warming。
- Client protocol：Today首次warming只保留quote並顯示準備狀態，不畫1～2根tail；以1／2／3／5秒cache-only small read觀察readiness。Backend轉ready或canonical history向左擴張時立即full recovery一次，完成後回latest-8；presentation cache拒絕warming與少於2點的snapshot。
- Command boundary：frontend viewer lease acquire會呼叫既有tracked Taiwan intraday bootstrap的per-symbol bounded enqueue；只在交易時段且coverage仍warming時發出，active job coalesce並以15秒success reuse收斂快速重複命令。Chart GET仍不enqueue、不等待provider、不寫DB。
- Source evidence：Backend Ready Gate matrix `56 passed`；Frontend TypeScript、targeted ESLint、pure contract Playwright `10 passed`與mocked browser recovery smoke `1 passed`。Browser smoke確認warming不畫tail，request序列為full／latest-8／full recovery，並在ready後返回latest-8，不需reload。Final Safe quick的architecture checker、architecture pytest、Backend compileall、Frontend TypeScript與`git diff --check`全數通過；log為`.tmp/validation/20260903-140208/`。
- Acceptance boundary：本follow-up未restart Backend、未做provider I/O或production DB mutation；3000 dev browser使用mocked API驗證Frontend protocol。Running OpenAPI adoption、2303／5347正式盤中baseline延遲、lease enqueue receipt與KGI/NStock Live仍pending；不得由Source／mock smoke推定Product accepted。

## 2026-09-03 Taiwan surface demand-driven loading convergence checkpoint

- Request ownership：台股個股畫面改為Bars-first，`/bars`與`/technical/{stock_id}/series`共用Backend-owned `session_scope`；Chart在Bars成功後立即ready。History Technical以response-local `series_revision`補齊；Current Session改用limit-independent `snapshot_revision`，Backend以完整snapshot計算後才依response `limit`裁切points，相同成功snapshot由Frontend去重。Bars owner保留bounded exact-snapshot cache，僅供後續Technical讀取剛回傳的同一revision；cache miss或revision改變仍走canonical read並fail closed。Technical Core只有在Chart ready且Technical面板進入需求範圍後才讀，並可明確省略volume pace的長歷史掃描。既有`/chart`保留為compatibility seam，沒有新增第二份session/freshness owner。
- Surface demand：SSR不再阻塞Chart bundle；Data tabs在使用者首次點擊後才讀且只載入active tab；Next Session／Overnight只在disclosure展開後讀。Ranking與Radar改為獨立lifecycle，Ranking watchlist排序不再逐檔計算完整signals；Radar先讀persisted snapshot，沒有snapshot時fail visible，不由consumer自行refresh或持久化。Quote depth完成後才啟動replay，避免同一切股瞬間並發兩條重讀。
- AI execution：明確target加Quote／Intraday／Freshness-only selection即使是general問句，也會鎖定`quote_only` focused reader；outward regression禁止退回完整`read_stock_context`。這只確認Source execution boundary，沒有執行AI POST、模型呼叫或MCP runtime驗收。
- Runtime／Product evidence：使用scheduler停用、lifespan停用的隔離Backend與production Frontend build做cache-only驗證；running OpenAPI已包含Bars／Technical的`session_scope`與Technical Core的`include_volume_pace`。同頁client-side切換2303、5347、2330時，Chart success分別為357／452／655 ms；100 ms內的target reads只包含Quote Depth、stock identity、Bars與realtime quote stream，Technical series在Bars後開始，沒有Institutional／Margin／Revenue／Next Session／Overnight等隱性讀取。Technical series人工延遲10秒或409時Chart仍ready；Radar 500時Ranking與Chart不受影響。Data chips明確點擊只讀chips coverage、shareholding與margin；secondary disclosures只在展開後讀各自cache-only endpoint。驗證時所有non-GET均被攔截，因此沒有lease、refresh、enqueue或DB side effect。
- Performance／validation：cache-only 20次量測的p95為Bars 217.10 ms、Technical series 136.28 ms、Quote Depth 287.89 ms、Technical Core 963.81 ms、Ranking group 45 1410.09 ms，全部0 errors。Backend affected matrix為`154 passed / 12 subtests`，AI outward focused regression另為`2 passed`；Frontend source-contract Playwright `19 passed`、targeted ESLint／TypeScript、production build均通過。Final safe quick的architecture checker、architecture pytest、Backend compileall、Frontend TypeScript與`git diff --check`全數通過，log為`.tmp/validation/20260903-154242/`。
- Corrective Source checkpoint：後續audit確認舊Current Session Technical在refresh `limit=8`時只用8根bar計算，且每次Bars成功都重算；上述舊timing不作corrective acceptance。修正後targeted Bars／Technical backend matrix為`31 passed`、Frontend TypeScript與19項source-contract Playwright通過；正式runtime、交易時段Live與Product latency仍待重新驗證。
- Acceptance boundary：本checkpoint接受Source與隔離Frontend stock-switch critical-path Product smoke；不是正式launcher、provider Live、交易時段持續量測、MCP／AI transport或整體Product closeout。所有77個watchlist groups目前都沒有persisted Radar snapshot，故Radar snapshot Product Gate仍pending；隔離Backend因停用lifespan而使readiness 503是預期限制。正式launcher維持停止；本輪未做provider I/O、production DB mutation、commit或push。

## 2026-09-03 Taiwan current-session technical correctness runtime checkpoint

- Current-session／coverage：explicit `read_current_session_bars()`在收盤後仍投影current-session coverage，current-date upper bound由calendar formal close截在13:31 exclusive。Snapshot gate把至少2點、coverage至少80%且missing不超過`max(3, ceil(expected/20))`的trailing window列為degraded；過度稀疏仍warming。因此TPEX 253／265與2330 262／265可如實顯示「走勢部分可用」，不再被隱藏或冒充complete。
- Quote／index correctness：public quote與realtime depth candidate read在provider priority及total bound前先套presentation trade date與allowed closing sessions，避免無關active rows擠掉同日MIS close-resolution evidence。Index summary刪除scalar-close合成OHLC fallback；completed daily或current-session aggregate只有在trade date與selected close一致時才附加，headline close／change／change percentage由同一resolved summary snapshot投影。
- AI／Frontend：AI index Today直接讀current-session Unified Bar，technical由同一series計算。Frontend actual-trade diagnostics只接受Backend `last_trade_price`，不再把session-close headline冒充last trade；Technical report是獨立secondary surface，首次read延後1.5秒，避免約1.7秒的同步計算與Bars首圖競爭，Technical series仍由Bars成功revision驅動。
- Runtime／Product：official launcher restart後，running Backend與Frontend proxy均載入本checkout。Browser逐一驗證加權45,857.66／265點、櫃買395.25／253點partial、3711為589／265點、2330為2,390／262點partial；指數另顯示canonical OHLC，3711與2330均顯示同日`TWSE MIS`收盤五檔快照及五組買賣價量。無fixture頁面內量測為加權655ms、櫃買538ms、3711 504ms、2330 522ms；2330→3711應用內stock switch首張Today為732ms，均低於Performance Plan的`<1s` target。硬式Next dev整頁導覽2.6～5.0秒是不同bootstrap口徑，不列為stock-switch gate通過證據。
- Validation／boundary：Backend affected matrix `119 passed, 1 warning`，warning僅Windows pytest cache權限；safe quick、targeted ESLint、Frontend TypeScript與isolated source-contract Playwright `9 passed`。Browser另以3711 Bars強制500驗證failure isolation：Chart為`error`，Technical endpoint仍回200並呈現`official_daily_finalized`。本checkpoint完成Source、Runtime adoption與收盤後cache-only Product evidence；沒有provider refresh、production DB mutation、AI／MCP transport、commit或push。正式交易時段new-minute freshness／persistence、5分鐘steady-state及快速切5檔p95仍是獨立Live gate。

## 2026-09-04 Taiwan intraday／auction session convergence checkpoint

- Event session：quote、depth與auction transaction依canonical observation `event_at`分類並保存session；request session不再覆寫evidence。`pre_open`與`opening_auction` applicability分離，08:20 evidence投影`waiting_preopen`，exact 13:30 closing match保存為`closing_auction`，13:33後confirmation只改變session-close projection status，不改寫原始event session。
- Consumer／finality：Ranking／Radar current overlay切到`TaiwanBarService` Unified Bar；Frontend移除MA／volume-MA硬編碼。Radar snapshot 404結束初始loading且不顯示錯誤，cache-only current computation延後交由既有60秒enhancement lifecycle，避免阻塞個股Chart critical path。Current-session AI `requested_at`先恢復typed aware datetime。Provisional Daily只接受同日、exchange authority、exact 13:30的session-close component，該component不製造1m bar且不冒充official daily。
- Frontend session：regular-session point截止改為13:25 exclusive，13:25～13:30保留closing-auction lane；圖表axis仍保留13:30 formal-close boundary。
- Source validation：affected Backend matrix `244 passed / 15 subtests`，touched Ruff全通過；Frontend targeted ESLint／TypeScript、source-contract Playwright `10 passed`及production build全通過。Radar最後修正後重跑Safe quick，architecture checker、architecture pytest、compileall、TypeScript與diff check亦通過，final log在`.tmp/validation/20260904-012556/`。
- Runtime／Product：official launcher已restart並確認8400／3000來自本checkout。最終效能證據另以8432 `--lifespan off` Backend與明確proxy至8432的43218 production Frontend隔離其他runtime負載；cold cache-only Bars為TAIEX 455ms、TPEX 108ms、3711 250ms、2330 122ms。實際browser Today為TAIEX 319ms／45,857.66／265點、TPEX 317ms／395.25／253點partial、3711 329ms／589／265點、2330 345ms／2,390／262點partial，皆符合`<1s` target。Access log確認四個Bars命中8432；Radar只有snapshot-only 404，沒有立即current recompute。
- AI runtime smoke：隔離3711 compact current-session context HTTP 200、約12秒，未再發生calendar字串傳入datetime-only boundary；此數字不屬於Chart `<1s` gate。
- Acceptance boundary：本checkpoint完成Source、Runtime adoption與收盤後cache-only Product gate。正式交易時段new-minute freshness、auction applicability、5分鐘steady-state及快速切5檔p95仍為獨立Live gate；本task不建立交易驗收排程或自動化，未commit／push。

## 2026-09-04 Taiwan coverage／formal-close product convergence checkpoint

- Hot Groups owner：`tw.intraday.group_snapshot.v2`改由完整 active `StockMaster` industry membership與enabled watchlist membership建立universe，再LEFT JOIN current state；`member_count = classified_count + unknown_count`可對帳，並分開揭露received／fresh／recent-trade coverage。Dashboard只把通過minimum member、observed與coverage policy的group納入正式ranking，raw snapshot仍保留不合格group與reason codes，缺observation不再讓整組消失。正式runtime的Dashboard由baseline `hot_groups=[]`恢復為8組；post-close stale／partial限制仍保留，未冒充decision usable。
- Calendar／outward contract：Taiwan calendar新增`previous_completed_trading_day`，不再讓legacy `previous_trading_day`同時承擔「當日」與「上一個完成交易日」；Index contribution REST schema保留component counts、coverage、estimate／reconciliation／confidence、status、reason codes與quality，API不再把Backend truth截掉。Production readback於17:28 Asia/Taipei回phase `post_close`、presentation date 2026-09-04、previous completed trading day 2026-09-03。
- Formal close presentation：Unified Bar Chart新增typed display-only `session_close_marker`，只由qualified formal-close component投影，不persist成1m Bar、不進regular-session coverage、不進technical input。3711與2330在Browser的Today header／最後更新都到2026-09-04 13:30；3711顯示588與266 display points，2330顯示2,410與263 display points。13:24 regular最後Bar仍保留，marker的`indicator_eligible=false`。
- Sparse post-close lifecycle：盤中excessive-gap sparse snapshot仍維持`warming`，避免不成熟tail冒充可用歷史；session completed後若已有至少2個觀測則投影`degraded`／`TW_CHART_SNAPSHOT_SPARSE_POST_CLOSE`，cold reload可顯示實際partial series。Browser重啟後TPEX顯示402.48、243點、`degraded`與「走勢部分可用」，不再永久白屏；TAIEX顯示46,551.13、265點、`ready`。2330維持`degraded`／`TW_CHART_SNAPSHOT_SPARSE`，沒有升成ready。
- Runtime／performance／Product：official launcher精確restart後，8400／3000採用本checkout。五次warm cache-only量測的Direct p95為TAIEX 15.23ms、TPEX 16.66ms、3711 86.32ms、2330 224.00ms；Frontend proxy p95依序27.44ms、16.02ms、48.23ms、178.56ms，全部低於Performance Plan `<1s` target。Browser DOM同時驗證四標的chart surface visible、point count與snapshot phase；TPEX的Browser gap是在驗收中發現後才修正並重啟重驗，不以API可用取代Product acceptance。
- Validation／boundary：final affected Backend matrix `242 passed / 6 subtests`，追加TPEX lifecycle targeted matrix `21 passed`；touched Ruff、Frontend TypeScript／targeted ESLint、focused source-contract Playwright `11 passed`與safe quick均通過。Index contribution component coverage在current runtime仍是TAIEX 0/1,073、TPEX 0/853並明確partial，TPEX 1m仍有22個missing buckets；正式交易時段30分鐘delivery telemetry、no-trade vs missing producer evidence、AI／MCP outward與全日preopen→post-close Live gate仍pending。本輪沒有provider refresh、production DB mutation、commit或push。

## 2026-09-04 Taiwan ETF Watchlist／OMI Ask convergence runtime checkpoint

- Ownership／scope：Dashboard Watchlist改讀Stock／ETF共用的cache-only canonical instrument projection，quote、qualified session close與official daily的選擇／freshness／semantics都留在Backend owner；Breadth仍沿用stock-only state。Producer只把active Watchlist ETF加入既有bounded universe，production intraday與session-close每輪上限為32，close tail另限3；沒有全市場ETF backfill或consumer-owned fallback。
- AI／MCP contract：`cache_only`只有在明確`require_live`時才會產生`live_requirement_not_satisfied`；qualified `official_close`是ready evidence，Data Quality會把單點Daily的partial進一步分類為`insufficient_history`。Frontend Today warming copy不再把in-flight history誤標為`missing`。HTTP `/api/ai/ask`對0050回`omi.decision.v4`、quality ready、freshness current；stdio MCP實際呼叫`omi.ask`對0056回`isError=false`、v4、ready／current與2026-09-04 official close。
- Runtime／Product：official launcher採用本checkout、root `.venv`、Backend 8400及Frontend 3000，`/omi-data` proxy指向8400。TAIEX、TPEX、3711、2330的Direct與proxy current-session Bars均回2026-09-04實際點位，final Direct warm p95為202／19／169／95ms。Browser DOM確認加權46,551.13／265點、櫃買402.48／243點、3711為588／266點、2330為2,410／263點，warm readiness依序302／302／302／467ms；全部低於`<1s` target，TPEX／2330的degraded狀態仍如實顯示。Watchlist Browser另確認0050 107.9、0056 55.4、006208 247.25、00981A 29.77，均為2026-09-04 official close。
- Validation／boundary：Backend relevant matrix `220 passed / 30 subtests`，architecture checker／pytest、compileall、Ruff與diff check通過；Frontend TypeScript、targeted ESLint、production build及source-contract Playwright `11 passed`。本checkpoint完成Source、正式Runtime adoption、HTTP／MCP與收盤後cache-only Product evidence；沒有provider refresh、production DB mutation、commit或push。正式交易時段new-minute delivery、長時間steady-state及完整session producer telemetry仍是後續Live gate。

## 2026-09-04 Taiwan post-close runtime convergence checkpoint

- Canonical owners：TPEX issued shares由typed daily table與official receipt parser擁有，daily persistence正常寫入；本次production backfill只解析既有raw receipt，不觸發provider I/O。Index contribution改讀release-qualified lightweight canonical rows並有專用read index；TAIEX／TPEX component coverage分別1073/1073與853/853，missing shares均為0，reconciliation皆within tolerance、quality high、decision usable。
- Outward contracts：ETF可走既有Dashboard detail contract並回傳typed instrument type；Technical calendar在post-close分別回傳previous completed 2026-09-03與latest completed 2026-09-04。Market chip只透過Index owner cache-only accessor投影同日close/change/percent/trade value；OMI index contribution不做external fetch或write。
- Account semantics：KGI actual position quantity不再相加total與component欄位；production 3711由240修正為120股，平均成本614.29、成本金額73,714.8。只有production account實際存在的3711可驗證，沒有虛構其他持倉作交叉證據。
- Runtime／Product：production DB採用`20260904_0080`，official launcher採用本checkout與root `.venv`。正式重啟後TAIEX／TPEX contribution direct reads分別78.842／63.596ms；四標的current-session Direct／Frontend proxy reads皆為HTTP 200且介於5.531–223.248／8.517–31.224ms；`omi.decision.v4` cache-only ask為4.144秒、低於20秒budget且兩個指數evidence皆ready。Browser DOM確認TAIEX 46,551.13、TPEX 402.48、3711 588及2330 2,410，均顯示2026-09-04實際資料；3711持倉顯示120股。
