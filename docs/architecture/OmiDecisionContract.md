...
- 台股：三大法人、融資券、券商分點、股權分散、營收、財報、跨市場 context、
  market breadth。
- 美股：quote/intraday、daily price、technical indicators／structure、SEC company facts，以及market scope的六指數`market.indices`；technical quality會保留raw price basis、corporate-action coverage、warm-up與decision-usability限制。US Index Daily沿用同一Canonical OHLCV與Shared Technical Engine，但volume與company corporate actions明示`not_applicable`；不得套用equity shares／corporate-action completeness規則，也不得以`0`取代不適用值。六指數aggregate固定組合`^GSPC`、`^DJI`、`^IXIC`、`^NDX`、`^SOX`、`^VIX`既有Market Truth，逐項保留selected provider/source、provider snapshot freshness、trade recency、current-session identity、fallback、comparison reference與truth revision；單項缺失維持partial，單項屬於current session但event stale時保留facts、將aggregate `is_current`與`decision_usable`降為false，不得由AI、MCP或Frontend自行選provider或補值。US `quote.snapshot`另保留Backend產生的`session_date_relation`，Data Quality只依該typed relation接受current Quote對previous completed Daily，不得用固定相差一天的heuristic放寬真正mismatch。
- 跨市場 public capability identity 使用 provider-neutral `omi.market.quote.snapshot.v1` 與 `omi.market.bars.v1`；`tw.quote.snapshot.v2`、`tw.intraday.bars.v2`只列入明示 compatibility versions，不得再宣稱為US canonical schema。Refresh tool compact result只代表acquisition diagnostics，AI consumer必須cache-only reread canonical Quote；projection與Data Quality必須monotonic保留upstream typed usability。`technical.indicators`的Daily依賴由resolution registry展開，US圖表所需逐點indicator series與1m／5m／15m aggregation均由Backend Shared Technical Engine產生，Frontend production path只投影，不重算成第二份truth。
