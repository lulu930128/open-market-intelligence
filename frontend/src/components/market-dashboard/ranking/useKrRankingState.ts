...
import type { KRWatchlistRankingRead } from "@/types/market";
import { useCallback, useEffect, useRef, useState } from "react";

export type KrRankBy = "none" | "change_pct" | "volume" | "close";
export type KrRankingErrorKind = "ranking" | "daily-refresh";
export type KrRankingLoadState = "idle" | "loading" | "success" | "error";

type UseKrRankingStateOptions = {
  active: boolean;
  groupId: number | null;
  refreshExecutionSettings: RefreshExecutionSettingsRead | null;
  startCompanionLoad: (input: { groupId: number; silent: boolean }) => void;
  onError: (kind: KrRankingErrorKind, error: unknown, groupId: number) => void;
};
...
export function useKrRankingState({
  active,
  groupId,
  refreshExecutionSettings,
  startCompanionLoad,
  onError,
}: UseKrRankingStateOptions) {
  const [rankBy, setRankBy] = useState<KrRankBy>("none");
    ...
    async (
      ...
      try {
        await requestBackfillJob(
          `/api/kr-market/watchlists/groups/${currentGroupId}/refresh-daily`,
          { method: "POST" },
          {
            include_children: true,
            enabled_only: true,
  ...
  const changeRankBy = useCallback((value: KrRankBy) => {
